    ...
    public addSubscription(jwtToken: string): Observable<TransactionConfirmation> {
        if (!this.subscriber) {
            this.subscriber = new Observable((subscriber) => {
                this.pushService
                    .requestSubscription({
                        // @ts-ignore - VITE_PUSH_PUBLIC_KEY is defined in .env
                        serverPublicKey: import.meta.env.VITE_PUSH_PUBLIC_KEY as string,
                    })
                    .then((subs) => {
                        this.subscription = subs;
                        this.registerSubscription(jwtToken, subs).subscribe({
                            // register notification url on server
                            next: () => {
                                this.pushService.messages.subscribe({
                                    // subscribe to WebWorker and wait for push messages
