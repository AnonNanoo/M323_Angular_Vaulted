...
export class AuthenticationResourceService {
    ...
    public login(model: LoginInfo): Observable<Credential | null> {
        return this.http
            .post<Credential>(getServerUrl('/auth/login'), JSON.stringify(model), getOptions())
            .pipe(
                tap((data) =>
                    console.info(`Login succeeded, retrieved JWT token '${data.token}'.`),
                ),
                catchError((err) => {
                    console.warn(
                        `Login failed, invalid credentials or duplicated user name detected: ${err?.message}`,
                        err.error,
                    );
                    return of(null);
                }),
